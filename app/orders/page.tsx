                            ...
                            <div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm border border-neutral-100 flex flex-col md:flex-row md:items-start justify-between gap-6">
                                <div className="flex-1 w-full">
                                        ...
                                        <ul className="flex flex-col gap-3">
                                            {order.items?.map((item: any, idx: number) => (
                                                <li key={idx} className="flex items-center gap-4">
                                                    ...
                                                    <div className="font-medium text-neutral-900 flex flex-col items-end gap-2">
                                                        <span>Rp {(item.price * item.quantity).toLocaleString("id-ID")}</span>
                                                        {order.status === "confirmed" && (
                                                            <button
                                                                onClick={() => setRatingModal({ show: true, productId: item.id, productName: item.name, rating: 5, submitting: false })}
                                                                className="text-xs text-amber-600 hover:text-amber-700 font-bold bg-amber-50 px-2 py-1.5 rounded-md border border-amber-200 flex items-center gap-1 transition"
                                                            >
                                    ...
                                    <div className="border-t border-neutral-100 pt-4 flex flex-col gap-2">
                                        <div className="flex flex-col gap-1 text-sm text-neutral-600 mb-2 p-3 bg-neutral-50 rounded-xl border border-neutral-100">
            ...
            {ratingModal.show && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl flex flex-col gap-4 text-center animate-scale-up">
                        <h2 className="text-xl font-bold text-neutral-900">Rate Product</h2>
                        <p className="text-sm text-neutral-500">How was your {ratingModal.productName}?</p>
                        ...
                        <div className="flex justify-center gap-2 my-2">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <button
                                    ...
                                    className={`transition ${star <= ratingModal.rating ? 'text-amber-500 scale-110' : 'text-neutral-200 hover:text-amber-200'}`}
                                >
                        ...
                        <div className="flex gap-3 mt-4">
                            <button
                                ...
                                className="flex-1 bg-neutral-100 text-neutral-600 font-bold py-3 rounded-xl hover:bg-neutral-200 transition"
                            >
                                Cancel
                            </button>
                            <button
